...
function showHighlightPopup(selection, event) {
  ...
  const button = highlightPopup.querySelector('.highlight-submit-btn');
  
  button.addEventListener('click', () => handleButtonClick(selection, input));
...
function handleHighlightQuery(context, question) {
  ...
  chrome.runtime.sendMessage({
    ...
    if (chrome.runtime.lastError || response?.error) {
      showResultNotification(chrome.runtime.lastError?.message || response.error);
      return;
    }
    chrome.runtime.sendMessage({
      action: "showHighlightResult",
